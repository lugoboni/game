...
public class RibWindow extends Table {
	...
	public RibWindow(String lbl, Skin skin, As as) {
		...
		tabla = new Table(Constants.SKIN);
	...
	public void act(float delta) {
		...
		Label Origem = new Label("ORIGEM", Constants.SKIN);
		Label Destino = new Label("DESTINO", Constants.SKIN);
		Label Aspath = new Label("ASPATH", Constants.SKIN);
					...
					if(!_rib_log_clone.isEmpty())
					{
						try {
							...
							for(BlocIP4Model b : temp.keySet())
							{
								...
								Label torig = new Label(model.get_ORIGEM().get_Full_Addr(), Constants.SKIN);
								Label tdest = new Label(model.get_DESTINO().get_Full_Addr(), Constants.SKIN);
								...
								Label laspath = new Label(saspath, Constants.SKIN);
